...
def generate_qrcode(label):
	...
	qr_codes = [
		generate_qrcode_internal(label_parts[0], np.array([0x00, 0x00, 0xFF])), # red
		generate_qrcode_internal(label_parts[1], np.array([0x00, 0xFF, 0x00])), # green
		generate_qrcode_internal(label_parts[2], np.array([0xFF, 0x00, 0x00])), # blue
	]
	result = qr_codes[0]
...
def generate_qrcode_internal(text, color):
	'''
